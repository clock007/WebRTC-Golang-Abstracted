...
func (pc *PeerConnection) OnTrack(f func(*TrackRemote, *RTPReceiver, Address_Info)) {
	pc.mu.Lock()
	defer pc.mu.Unlock()
	pc.onTrackHandler = f
}
...
func (pc *PeerConnection) setDescription(sd *SessionDescription, op stateChangeOp) error { //nolint:gocognit
	...
	case NewSDPType(sd.Type.String()) == SDPType(Unknown):
		...
		return &rtcerr.TypeError{Err: fmt.Errorf("%w: '%d' mmmm is not a valid enum value of type SDPType", errPeerConnSDPTypeInvalidValue, sd.Type)}
	}
